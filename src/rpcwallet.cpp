...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "A negative timeout disables relocking.\n"
    ...
    int64 nTimeout = params[1].get_int64();
    if (nTimeout > 999999999) // to avoid crash in the relock thread
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Maximum timeout is 999999999");
    ...
    if (nTimeout >= 0)
    {
        ...
        pargCleanWallet->nSleepTime = nTimeout;
